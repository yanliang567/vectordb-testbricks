...
- `milvus-log-level` (default `debug`)
- `index-engine-contract-mode` (protected metadata; default `none`)
- `index-engine-capability` (protected metadata; default `none`)
- `index-engine-qualification-status` (protected metadata; default `not_applicable`)
...
All standalone and cluster upgrade/rollback templates default Milvus to
`log.level: debug`. The protected `milvus-log-level` workflow parameter is
passed to the Operator CR or Helm values renderer, retained by image/config
patches, and included in the deployment topology/config evidence. This keeps
base, target, and rollback logs diagnostic by default without per-scenario
configuration.

The v10/v4 and v11/v4 index engine scenarios are target-only gates. Their base
and rollback phases use `schema_matrix_2_6.yaml` and the default index engine
selection because the fix tracked by
[#52767](https://github.com/milvus-io/milvus/issues/52767) cannot be present in
released v3.0.0. Only the target phase applies the requested vector/scalar
engine versions and validates the dedicated forward matrix. Before rollback,
both the dedicated forward collections and the `_after_upgrade` phase-DML
collections are dropped and excluded from rollback compatibility validation;
the rollback-safe baseline collections still cover data integrity,
serviceability, pressure, and rollback behavior. After rollback, the phase
checkpoint verifies that target-only collections are absent rather than trying
to load target-format index artifacts on the baseline image.

These scenarios are declared through the manifest v2 `index_engine_contract`.
`target_only` compiles to a rollback-safe base matrix, target-only engine
versions, target-created collection deletion, and no target-format validation
after rollback.
`round_trip` compiles to the capability matrix and engine versions in all three
phases, retains target-created data, and requires it after rollback. A formal
`round_trip` scenario is accepted only when `capability_qualifications` binds a
passed capability result and topology-specific `argo://` or `https://` evidence
to the exact digest-pinned base and rollback image. Runtime overrides cannot
bypass this binding. Target-only rollback-safe matrices must contain only
`compat_mode: rollback_safe` schemas and no `IndexEngine*` capability.

The three contract parameters listed above are protected values generated by
the manifest renderer. Registered workflows reject drift, while unregistered
runs may only claim `none/none/not_applicable`. The contract mode directly
controls phase-new collection cleanup and rollback checkpoint semantics;
compiled low-level matrix and lifecycle flags continue to control the other
index paths. Non-index and `round_trip` scenarios retain their prior behavior.
...
- after rollback in `none` or `round_trip` mode: first validate the after-upgrade
  phase checkpoint, proving the baseline `50000000` range and upgrade-created
  `60000000` collections survived rollback. Only after that succeeds does the
  workflow run insert/upsert/delete on baseline collections again, carry the
  upgrade-created collections forward for another DML/DQL round, create one
  `${collection-prefix}_after_rollback` collection per schema, then query/search
  all of them;
- after rollback in `target_only` mode: reload/query the checkpoint's baseline
  group, require every upgrade-created `60000000` collection to be absent, and
  skip carried-collection DML. Baseline DML/DQL and creation/validation of the
  `${collection-prefix}_after_rollback` group still run normally. A residual
  target-only collection fails with
  `PHASE_CHECKPOINT_TARGET_ONLY_COLLECTION_PRESENT`.

Default deterministic data scale for `none` and `round_trip` contracts:
...
For `target_only`, the after-upgrade and baseline counts are unchanged, but the
upgrade-new group is removed before rollback. Its after-rollback contract is an
absence check; only baseline collections receive the second DML round and the
rollback-new group is created.
...
rollback writes. It validates both `50000000` and `60000000` data for
`none/round_trip`, or validates `50000000` data plus `60000000` collection
absence for `target_only`, before the `70000000` / `80000000` ranges are
inserted. The
...
| 3.0.1 Vortex -> 3.0.1 Vortex -> 3.0.1 Vortex | `milvus-standalone-3-0-upgrade-rollback` and `milvus-cluster-upgrade-rollback` | Vortex TEXT LOB data, serviceability, and pressure | Vortex baseline self-compatibility. LoonFFI/Vortex stay enabled in every phase. |
| 3.0.0 legacy -> 3.0.1 + LoonFFI/Vortex -> 3.0.1 + LoonFFI/Vortex | standalone and cluster workflows | #52340 upgrade path | Enables LoonFFI/Vortex at upgrade; rollback stays on 3.0.1 Vortex and must not return to v3.0.0. The 3.0.1 dual reader handles mixed legacy + Vortex segments. |
| 3.0.1 Vortex -> 3.0.1 + JSON Shredding + Vortex -> 3.0.1 + JSON Shredding + Vortex | standalone and cluster workflows | JSON Shredding plus Vortex | JSON-heavy forward data written under Vortex; both features stay enabled through rollback. |
| 3.0.1 legacy -> 3.0.1 + LoonFFI(storage v3) -> 3.0.1 legacy | standalone and cluster workflows | LoonFFI-only rollback safety | LoonFFI (storage v3) without Vortex; validates storage v3 -> v2 rollback readability under the dual-engine reader. |
| 3.0.1 legacy -> 3.0.1 + LoonFFI/Vortex -> 3.0.1 legacy | `milvus-standalone-3-0-upgrade-rollback` | Vortex disable at rollback | Disabling Vortex at rollback; the 3.0.1 dual-format reader still reads Vortex segments. |
| 3.0.1 legacy -> 3.0.1 + LoonFFI/Vortex -> 3.0.1 + LoonFFI(no Vortex) | `milvus-standalone-3-0-upgrade-rollback` | Vortex disable at rollback | Disabling Vortex at rollback while keeping LoonFFI (S4 -> S2); dual-format reader still reads Vortex segments. |
| 2.6.18 -> latest 3.0 LoonFFI/vortex -> latest 2.6 | `milvus-standalone-2-6-upgrade-rollback` | Negative/observe-only | Not a promoted gate. This documents the unsafe 2.6 rollback boundary (2.6 is storage v2 only) and must not be treated as supported. |
