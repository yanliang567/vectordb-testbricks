...
| 3.0.0 legacy -> 3.0.1 + LoonFFI/Vortex -> 3.0.1 + LoonFFI/Vortex | standalone and cluster workflows | #52340 upgrade path | Pre-release candidate, not a promoted gate. Enables LoonFFI/Vortex at upgrade; rollback stays on 3.0.1 Vortex and must not return to v3.0.0. Mixed legacy + Vortex segments must be confirmed on real hardware before promotion. |
...
| 3.0.1 legacy -> 3.0.1 + LoonFFI(parquet) -> 3.0.1 legacy | standalone and cluster workflows | LoonFFI-only rollback safety | Pre-release candidate, not a promoted gate. Isolates the LoonFFI engine from the Vortex format; whether L=1 parquet data is readable by the L=0 legacy engine must be confirmed before promotion. |
...
| 3.0.1 legacy -> 3.0.1 + LoonFFI/Vortex -> 3.0.1 legacy | `milvus-standalone-3-0-upgrade-rollback` | Negative/observe-only | Disabling Vortex at rollback after writing Vortex data is an unsafe in-place format downgrade. |
| 3.0.1 legacy -> 3.0.1 + LoonFFI/Vortex -> 3.0.1 + LoonFFI(no Vortex) | `milvus-standalone-3-0-upgrade-rollback` | Negative/observe-only | Disabling Vortex at rollback while keeping LoonFFI (S4 -> S2) is an unsafe in-place format downgrade. |
