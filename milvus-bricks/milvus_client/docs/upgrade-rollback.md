...
those forward collections are dropped and excluded from rollback validation;
the rollback-safe baseline collections still cover data integrity,
serviceability, pressure, and rollback behavior.
...
versions, forward-data deletion, and no forward validation after rollback.
...
The three contract parameters listed above are report metadata generated by the
manifest renderer. Registered workflows reject drift, while unregistered runs
may only claim `none/none/not_applicable`. The execution DAG continues to use
the compiled low-level matrix and lifecycle flags, so introducing the contract
does not change non-index upgrade/rollback paths.
