...
def validate_deploy_profile(profile: dict[str, Any], source: str = "<profile>") -> None:
    ...
    if deployer == "helm":
        ...
        if not isinstance(helm, dict):
            raise ValueError(f"{source}: helm settings are required when deployer is helm")
        for field in ("repo_name", "repo_url", "chart"):
            if not helm.get(field):
                raise ValueError(f"{source}: helm.{field} is required")
        if mode != "cluster":
            raise ValueError(f"{source}: helm deployer is currently supported only for cluster profiles")
...
def render_milvus_helm_values(
    *,
    ...
    values["cluster"] = _deep_merge(values.get("cluster", {}), {"enabled": profile["mode"] == "cluster"})
    values["image"] = _deep_merge(
        values.get("image", {}),
        {"all": {"repository": repository, "tag": tag, "pullPolicy": image_pull_policy}},
    )
    ...
    values.setdefault("commonLabels", {}).update(labels or {})
    values.setdefault("commonAnnotations", {}).update(annotations or {})
    values.setdefault("extraConfigFiles", {})["upgrade-rollback.yaml"] = yaml.safe_dump(
        {
            "workflow": {"name": annotations.get("zilliz.com/workflow-name") if annotations else name},
            "deployment": {
                "namespace": namespace,
    ...
    storage_values = dependencies.get("storage", {}).get("inCluster", {}).get("values", {})
    if etcd_values:
...
def helm_deploy_topology_summary(profile: dict[str, Any], values: dict[str, Any], *, image: str, version: str) -> dict[str, Any]:
    components = {}
    for profile_key, helm_key in _HELM_COMPONENT_KEYS.items():
